...
class AnomalyDetectionService
{
    ...
    public function detectVolumeAnomaly(int $stockId): ?array
    {
        try {
            ...
            $mean = array_sum($historicalVolumes) / count($historicalVolumes);
            $variance = 0;
            foreach ($historicalVolumes as $vol) {
                $variance += pow($vol - $mean, 2);
            }
            $stdDev = sqrt($variance / count($historicalVolumes));
    ...
    public function detectPriceAnomaly(int $stockId): ?array
    {
        try {
            ...
            $mean = array_sum($historicalReturns) / count($historicalReturns);
            $variance = 0;
            foreach ($historicalReturns as $ret) {
                $variance += pow($ret - $mean, 2);
            }
            $stdDev = sqrt($variance / count($historicalReturns));
